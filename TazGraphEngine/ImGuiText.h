...
namespace TazGraphEngine {
	...
	inline void drawTextAtWorldPositionPerspective(const glm::vec3& worldPos, const char* text,
		...
		glm::vec4 clipPos = camera->getCameraMatrix() * glm::vec4(worldPos, 1.0f);
		...
		if (clipPos.w > 0.0f) {
			...
			glm::vec2 screenPos;
			screenPos.x = pos.x + (clipPos.x * 0.5f + 0.5f) * viewportSize.x;
			screenPos.y = pos.y + (-clipPos.y * 0.5f + 0.5f) * viewportSize.y;
