#include "AudioEngine.h"
	...
	void SoundEffect::play(int loops) {
		/*if (Mix_PlayChannel(-1, _chunk, loops)) {
			if (Mix_PlayChannel(0, _chunk, loops)) {
				TAZ_ERROR("Mix_PlayChannel error: " + std::string(Mix_GetError()));
	...
	void AudioEngine::init() {
		if (_isInitialized) {
			TAZ_ERROR("Tried to initialize AudioEngine Twice!\n");
		}

		// Parameter can be a bitwise combination of MIX_INIT_FAC,
		//MIX_INIT_MOD, MIX_INIT_MP3, MIX_INIT_OGG
		if (Mix_Init(MIX_INIT_MP3 | MIX_INIT_OGG) == -1) {
			TAZ_ERROR("Mix_Init error: " + std::string(Mix_GetError()));
		}

		if (Mix_OpenAudio(MIX_DEFAULT_FREQUENCY, MIX_DEFAULT_FORMAT, 2, 1024)) {
			TAZ_ERROR("Mix_OpenAudio error: " + std::string(Mix_GetError()));
		}
	...
	SoundEffect AudioEngine::loadSoundEffect(const std::string& filePath) {
		...
		if (it == _effectMap.end()) {
			...
			//Check for errors
			if (chunk == nullptr) {
				TAZ_ERROR("Mix_LoadWAV error: " + std::string(Mix_GetError()));
			}
			effect._chunk = chunk;
