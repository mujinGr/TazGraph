...
void Manager::applyStep(const SimulationStep& step)
{
	// Nodes

	for (auto& group : groupedNodeEntities) {
		auto& entitiesInGroup = group;

		for (NodeEntity* node : entitiesInGroup) {
			if (node->hasComponent<TransformComponent>()) {
				auto& tc = node->GetComponent<TransformComponent>();
				tc.position = step.positions[i];
				tc.size = glm::vec3(step.nodeSizes[i] * 10.0f,
					step.nodeSizes[i] * 10.0f,
					0.0f);
			}
			if (node->hasComponent<Rectangle_w_Color>()) {
				node->GetComponent<Rectangle_w_Color>().color = step.nodeColors[i];
			}
		}
	}
	...
	for (auto& group : groupedLinkEntities) {
		...
		for (LinkEntity* link : entitiesInGroup) {
			if (link->hasComponent<Line_w_Color>()) {
				auto& lwc = link->GetComponent<Line_w_Color>();
				lwc.src_color = step.linkColors[i];
				lwc.dest_color = step.linkColors[i];
				lwc.width = step.linkWidths[i];
			}
		}
	}

	// Paths (example)
	for (auto& pathHolder : getGroup<EmptyEntity>(Manager::groupPathLinksHolder)) {

		if (pathHolder && pathHolder->hasComponent<PathLinkerComponent>()) {
			auto& plc = pathHolder->GetComponent<PathLinkerComponent>();
			plc.color = step.paths[0].first;
			plc.width = step.paths[0].second;
		}
	}
}
