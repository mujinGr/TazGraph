...
\begin{DoxyItemize}
\item \doxylink{fileFormats}{File Formats}
\item \doxylink{pythonCommands}{Python Commands}
\end{DoxyItemize}\hypertarget{dev_guide_autotoc_md0}{}\doxysubsubsection{\texorpdfstring{Core Architecture\+:}{Core Architecture:}}\label{dev_guide_autotoc_md0}
...
The ECS is managed by \texorpdfstring{$\ast$}{*}\texorpdfstring{$\ast$}{*}{\ttfamily GECSManager}\texorpdfstring{$\ast$}{*}\texorpdfstring{$\ast$}{*}, which handles creation, removal, update, and drawing of entities. \doxylink{classEntity}{Entity} processing is based on {\bfseries{groups}} ({\ttfamily Entity\+Groups}) for rendering and update.\hypertarget{dev_guide_autotoc_md1}{}\doxysubsubsection{\texorpdfstring{Scene System}{Scene System}}\label{dev_guide_autotoc_md1}
...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{dev_guide_autotoc_md2}{}\doxysubsubsection{\texorpdfstring{Graph Rendering Pipeline}{Graph Rendering Pipeline}}\label{dev_guide_autotoc_md2}
All rendering logic is handled in {\ttfamily Graph\+::draw()}.\hypertarget{dev_guide_autotoc_md3}{}\doxyparagraph{\texorpdfstring{1. Shader Setup}{1. Shader Setup}}\label{dev_guide_autotoc_md3}
...
\end{DoxyCode}
\hypertarget{dev_guide_autotoc_md4}{}\doxyparagraph{\texorpdfstring{2. Render Pass Initialization}{2. Render Pass Initialization}}\label{dev_guide_autotoc_md4}
...
\end{DoxyCode}
\hypertarget{dev_guide_autotoc_md5}{}\doxyparagraph{\texorpdfstring{3. Render Batching (\+Multithreaded)}{3. Render Batching (Multithreaded)}}\label{dev_guide_autotoc_md5}
...
\end{DoxyCode}
\hypertarget{dev_guide_autotoc_md6}{}\doxyparagraph{\texorpdfstring{4. Instance Rendering}{4. Instance Rendering}}\label{dev_guide_autotoc_md6}
...
{\ttfamily Not all \doxylink{classEntity}{Entity} Components can draw with all renderers (e.\+g. Sprite \doxylink{classComponent}{Component} is used to specifically draw for \doxylink{classPlaneModelRenderer}{Plane\+Model\+Renderer}). The draw function in the component indicates the renderers used\+: {\ttfamily void draw(size\+\_\+t v\+\_\+index,}{\ttfamily \doxylink{classPlaneModelRenderer}{Plane\+Model\+Renderer}}{\ttfamily \& batch, \doxylink{classTazGraphEngine_1_1Window}{Taz\+Graph\+Engine\+::\+Window}\& window) \{ ... \} }}\hypertarget{dev_guide_autotoc_md7}{}\doxysubsubsection{\texorpdfstring{Custom Renderers}{Custom Renderers}}\label{dev_guide_autotoc_md7}
...
In the following folder\+: {\ttfamily Taz\+Graph\+Engine/\+Renderers/\+Plane\+Renderers/\+Plane\+Lit\+Text\+Renderer} use one of the existing renderers as a base, then write glsl shader to match the renderer under {\ttfamily Taz\+Graph/\+Src/\+Shaders}, implement a {\ttfamily draw} function in a component that uses that renderer and then pass that renderer as argument for {\ttfamily render\+Batch} in {\ttfamily Graph\+::draw()}.\hypertarget{dev_guide_autotoc_md8}{}\doxysubsubsection{\texorpdfstring{Graph Loaders (\+WIP)}{Graph Loaders (WIP)}}\label{dev_guide_autotoc_md8}
Currently Taz\+Graph has the ability to process txt and python files with a specific format. Developers are able to implement their own processing of files, by creating more functions inside {\ttfamily Taz\+Graph/\+Src/\+Map} \input{fileFormats}
