...
public: // it is like it has init that creates Animator Component since it inherits it
	...
	void init() override {
		...
		if (!entity->hasComponent<TransformComponent>()) {
			entity->addComponent<TransformComponent>(glm::vec2(-TextureManager::getInstance().Get_GLTexture("graphnetwork")->width / 2, -TextureManager::getInstance().Get_GLTexture("graphnetwork")->height/2), Manager::actionLayer,
				glm::ivec2(
